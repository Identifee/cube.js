...
const setupScheduler = ({ repository, useOriginalSqlPreAggregations, skipAssertSecurityContext }) => {
    ...
    class CubejsServerCoreDisabledRefreshTimer extends src_1.CubejsServerCore {
...
describe('Refresh Scheduler', () => {
    ...
    afterAll(async () => {
        ...
        await (0, shared_1.pausePromise)(250);
    });
    ...
    test('Drop without touch', async () => {
        ...
        const ctx = { authInfo: { tenantId: 'tenant1' }, securityContext: { tenantId: 'tenant1' }, requestId: 'XXX' };
        for (let i = 0; i < 1000; i++) {
            const refreshResult = await refreshScheduler.runScheduledRefresh(ctx, { concurrency: 1, workerIndices: [0], timezones: ['UTC'] });
            if (refreshResult.finished) {
                break;
            }
        }
