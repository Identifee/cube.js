...
const RefreshScheduler_1 = require("../../src/core/RefreshScheduler");
const CompilerApi_1 = require("../../src/core/CompilerApi");
const OrchestratorApi_1 = require("../../src/core/OrchestratorApi");
const schemaContent = `
cube('Foo', {
...
`;
const repositoryWithPreAggregations = {
    localPath: () => __dirname,
...
class MockDriver extends query_orchestrator_1.BaseDriver {
    constructor() {
        super();
        this.tables = [];
        this.createdTables = [];
...
let testCounter = 1;
const setupScheduler = ({ repository, useOriginalSqlPreAggregations }) => {
    const serverCore = new src_1.CubejsServerCore({
        dbType: 'postgres',
        apiSecret: 'foo',
    });
    const compilerApi = new CompilerApi_1.CompilerApi(repository, async () => 'postgres', {
        compileContext: {
    ...
    const orchestratorApi = new OrchestratorApi_1.OrchestratorApi(() => mockDriver, (msg, params) => console.log(msg, params), {
        contextToDbType: async () => 'postgres',
        contextToExternalDbType() {
            return 'cubestore';
        },
    ...
    const refreshScheduler = new RefreshScheduler_1.RefreshScheduler(serverCore);
...
describe('Refresh Scheduler', () => {
    ...
    test('Round robin pre-aggregation refresh by history priority', async () => {
        ...
        const { refreshScheduler, mockDriver, } = setupScheduler({ repository: repositoryWithPreAggregations, useOriginalSqlPreAggregations: true });
        const result1 = [
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            expect(mockDriver.createdTables).toEqual(ramda_1.default.take(mockDriver.createdTables.length, result1));
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            expect(mockDriver.createdTables).toEqual(ramda_1.default.take(mockDriver.createdTables.length, result1.concat(result2)));
            if (refreshResult.finished) {
                break;
            }
        }
    ...
    test('Cache only pre-aggregation partitions', async () => {
        process.env.CUBEJS_EXTERNAL_DEFAULT = 'false';
        process.env.CUBEJS_SCHEDULED_REFRESH_DEFAULT = 'false';
        const { refreshScheduler, } = setupScheduler({ repository: repositoryWithPreAggregations, useOriginalSqlPreAggregations: true });
        const ctx = { authInfo: { tenantId: 'tenant1' }, securityContext: { tenantId: 'tenant1' }, requestId: 'XXX' };
        for (let i = 0; i < 100; i++) {
            try {
    ...
    test('Round robin pre-aggregation with timezones', async () => {
        const { refreshScheduler, mockDriver, } = setupScheduler({ repository: repositoryWithPreAggregations });
        const result = [
            { tableName: 'stb_pre_aggregations.foo_first20201231', timezone: 'UTC', fromTable: 'foo_tenant1' },
        ...
        const queryIteratorState = {};
        for (let i = 0; i < 1000; i++) {
            const refreshResult = await refreshScheduler.runScheduledRefresh(ctx, { concurrency: 2, workerIndices: [0], timezones: ['UTC', 'America/Los_Angeles'], queryIteratorState });
            expect(mockDriver.createdTables).toEqual(ramda_1.default.take(mockDriver.createdTables.length, result.filter((x, qi) => qi % 2 === 0)));
            if (refreshResult.finished) {
                break;
            }
        }
        for (let i = 0; i < 1000; i++) {
            const refreshResult = await refreshScheduler.runScheduledRefresh(ctx, { concurrency: 2, workerIndices: [1], timezones: ['UTC', 'America/Los_Angeles'], queryIteratorState });
            const prevWorkerResult = result.filter((x, qi) => qi % 2 === 0);
            expect(mockDriver.createdTables).toEqual(ramda_1.default.take(mockDriver.createdTables.length, prevWorkerResult.concat(result.filter((x, qi) => qi % 2 === 1))));
            if (refreshResult.finished) {
                break;
            }
        }
        expect(mockDriver.createdTables).toEqual(result.filter((x, qi) => qi % 2 === 0).concat(result.filter((x, qi) => qi % 2 === 1)));
    ...
    test('rollupJoin scheduledRefresh', async () => {
        ...
        const { refreshScheduler } = setupScheduler({ repository: repositoryWithRollupJoin, useOriginalSqlPreAggregations: true });
        const ctx = { authInfo: { tenantId: 'tenant1' }, securityContext: { tenantId: 'tenant1' }, requestId: 'XXX' };
        for (let i = 0; i < 1000; i++) {
            try {
...
//# sourceMappingURL=RefreshScheduler.test.js.map